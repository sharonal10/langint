...
def get_param_norm(m: nn.Module):
    param_norm = [torch.linalg.norm(p) for p in m.parameters()]
    if len(param_norm) == 0:
        return None
    return torch.stack(param_norm).mean()
...
def get_grad_norm_safe(m: nn.Module, verbose=True):
    grad_norm = []
