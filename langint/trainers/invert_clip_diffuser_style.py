    ...
    def _visualize_core(self, data, prefix=None):
        if 'dataset' in data:
            ...
            if self.writer is not None:
                self.vi_helper.dump_table(self.vi, [[str([sublist for sublist in dataset.ph_words_all[i]]), dataset.gt_prompts[i//dataset.num_data_per_prompt], dataset.blip_fruits[i//dataset.num_data_per_prompt], dataset.blip_colors[i//dataset.num_data_per_prompt]] for i in range(len(dataset.ph_words_all))],
                                      table_name='', col_names=[f'{prefix}/ph_words', f'{prefix}/gt_prompt', f'{prefix}/blip_fruit', f'{prefix}/blip_color']) 
        ...
        with torch.no_grad():
            if 'val' in prefix:
                ...
                #prints out text 
                if self.writer is not None:
                    self.vi_helper.dump_table(self.vi, [[data['gt_prompt'][0]], [data['prompt'][0] + f' (blip_fruit: {data["blip_fruit"][0]})' + f' (blip_color: {data["blip_color"][0]})'], [fruit_prompts[0]], [color_prompts1[0]]],
                ...
                for i in range(len(data['all_gt_colors'])):
                    ...
                    if curr_ph_color != data['ph_color'][0]:
                ...
                if self.writer is not None:
                    self.vi_helper.dump_table(self.vi, [[data['gt_prompt'][i], data['prompt'][i], data['fruit_prompt'][i]] for i in range(len(data['gt_prompt']))],
                                            table_name='', col_names=[f'{prefix}/gt', f'{prefix}/inverted', f'{prefix}/inverted_ph_only'])
            else:
                ...
                if self.writer is not None:
                    self.vi_helper.dump_table(self.vi, [[out['processed_prompts'][ind], data['gt_prompt'][ind], data['blip_fruit'][ind], data['blip_color'][ind]] for ind in range(len(data['prompt']))],
                                            table_name='', col_names=[f'{prefix}/prompt', f'{prefix}/gt_prompt', f'{prefix}/blip_fruit', f'{prefix}/blip_color'])
