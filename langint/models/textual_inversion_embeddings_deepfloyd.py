...
class Embeddings(nn.Module):
            ...
            for word in words:
                if word in self.trainable_embeddings:
                    ...
                    if int(number) % 3 == 0: 
                        ...
                        blip_word_dict[word] = blip_mat_emb[0]
                    elif int(number) % 3 == 2:
    ...
    def _get_text_embeddings(self, texts, t5, tokenizer, model, data):
        ...
        return text_encoder_embs, text_tokens_and_mask, fruit_blip_loss, mat_blip_loss, color_blip_loss
    ...
    def forward(self, data, return_all=False, return_pre_gumbel=False) -> Dict[str, torch.Tensor]:
        ...
        text_encoder_embs, text_tokens_and_mask, fruit_blip_loss, mat_blip_loss, color_blip_loss = self._get_text_embeddings(texts, t5, tokenizer, model, data)
    ...
    def _inference_forward(self, data):
        ...
        for ph_word, emb in zip(ph_words_by_prompt, x):
            ...
            ph_fruit, ph_mat, ph_color = ph_word 
            emb_fruit, emb_mat, emb_color = emb
            if ph_fruit != None:
                temp_ph_word_dict[ph_fruit] = emb_fruit.to(torch.bfloat16)
            if ph_mat != None:
